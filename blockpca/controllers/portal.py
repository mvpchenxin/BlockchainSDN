...
__all__ = ['UserControllerPortal']

class NamespaceController(BaseCtller):
    def __init__(self, nselem, name):
        self.nselem = nselem
        self.name = name

    @expose('blockpca.templates.portal')
    def _default(self):
        """Handle the user's profile page."""
        return dict(page='profile', view=match_field(self.nselem), name=self.name)

class NamespaceControllerEdit(BaseCtller):
    ...
    @expose('blockpca.templates.edit')
    def _default(self):
...
class UserControllerPortal(BaseCtller):
    ...
    def _lookup(self, *args):
        ...
        if edit:
            return NamespaceControllerEdit(root, xpath, user, record), args

        return NamespaceController(node, name), args
