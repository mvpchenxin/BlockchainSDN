...
from blockpca import model
from blockpca.model import DBSession
from blockpca.config.app_cfg import AdminConfig
from tgext.admin.controller import AdminController

from blockpca.lib.base import BaseCtller
from blockpca.controllers.error import ErrorController
from blockpca.controllers.portal import UserControllerPortal
from blockpca.controllers.symptom import SymptomController

__all__ = ['RootCtller']

class RootCtller(BaseCtller):
    admin = AdminController(model, DBSession, config_type=AdminConfig, translations=config.sa_auth.translations)
    error = ErrorController()
    p = UserControllerPortal()
    symptom = SymptomController()

    def _before(self, *args, **kw):
        tmpl_context.project_name = "blockpca"

    @expose('blockpca.templates.index')
    def index(self):
    ...
    @expose('blockpca.templates.login')
